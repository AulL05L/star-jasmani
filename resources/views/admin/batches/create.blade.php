@extends('layouts.app')
@section('title', 'Buat Angkatan Baru')

@section('content')
<div class="min-h-screen bg-black text-white p-6 lg:p-8 max-w-2xl mx-auto">

    <div class="mb-8">
        <a href="{{ route('admin.batches.index') }}" class="text-gray-500 hover:text-white text-sm flex items-center gap-2 mb-4 transition-colors">
            <i class="fa-solid fa-arrow-left"></i> Kembali ke Daftar Angkatan
        </a>
        <h1 class="text-2xl font-extrabold tracking-tighter">Buat <span class="text-red-500">Angkatan Baru</span></h1>
        <p class="text-gray-500 text-sm mt-1">Parameter sesi tes bisa ditambahkan setelah angkatan dibuat.</p>
    </div>
    ...
    <form action="{{ route('admin.batches.store') }}" method="POST" class="bg-gray-950 border border-gray-800 rounded-2xl p-6 space-y-5">
        @csrf

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                <label class="block text-gray-400 text-xs uppercase tracking-widest font-bold mb-2">Nama Angkatan *</label>
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Batch 1"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <select name="institution_code"
                    ...
                    <option value="{{ $inst }}" {{ old('institution_code','POLRI') === $inst ? 'selected' : '' }}>{{ $inst }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="max_parameters"
                    class="w-full bg-gray-900 border border-gray-700 focus:border-red-700 rounded-xl px-4 py-3 text-white text-sm outline-none transition-colors">
                    @foreach([2,3,4] as $n)
                    <option value="{{ $n }}" {{ old('max_parameters',4) == $n ? 'selected' : '' }}>{{ $n }} Parameter</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="description" rows="3" placeholder="Catatan tambahan tentang angkatan ini..."
                class="w-full bg-gray-900 border border-gray-700 focus:border-red-700 rounded-xl px-4 py-3 text-white text-sm outline-none transition-colors resize-none">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit"
                ...
                <i class="fa-solid fa-plus mr-2"></i> Buat Angkatan
            </button>
