...
class SamaptaScore extends Model
{
    ...
    private function calcScorePushup(?int $reps, string $gender): ?float
    {
        ...
        // ✅ CORRECTED: Push-Up Putri max 37 reps (sebelumnya salah pakai data sit-up)
        $tablePutri = [
    ...
    {
        if ($reps === null) return null;
        // ✅ CORRECTED: Added 7→2 and 6→1 entries
        $tablePutra = [
        ...
        // ✅ CORRECTED: Fixed wrong scores (49→96, 27→29, 24→21, 19→6, 18→3, 17→1) + removed extra entries
        $tablePutri = [
            ...
            [26,26],[25,24],[24,21],[23,19],[22,15],[21,12],[20,10],[19,6],
            [18,3],[17,1],
        ];
        return $this->lookupTable($reps, $gender === 'pria' ? $tablePutra : $tablePutri);
    }
